...
window = tk.Tk()
window.title("Tic Tac Toe by MUSKAN GUPTA")

# Create board
def create_board():
    for i in range(3):
        for j in range(3):
            button = tk.Button(window, text="", font=("Arial", 50), height=1, width=3, bg="light goldenrod", command=lambda row=i, col=j: handle_click(row, col))
